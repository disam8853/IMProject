...
class Primal():
    ...
    def get_primal_feasible(self, mu, a_wlk):
        mu1, mu2, mu3, mu4, mu5, mu6, mu7, mu8, mu9, mu10, mu11 = mu
        self.initialize_decision_variables()
    ...
    def calDelay(self):
        ...
        # tau_l
        for l in range(self.link_set.length()):
            self.tau[l] = (1/self.link_set.links[l].svcRate) * (1-self.gamma[l][self.prioritylv-1])

        # q_lk
        for l in range(self.link_set.length()):
            for k in range(self.prioritylv):
                self.q[l][k] = self.tau[l] / (self.gamma[l][k-1]*self.gamma[l][k])        
        
        # t_lk
        for l in range(self.link_set.length()):
            for k in range(self.prioritylv):
                self.t[l][k] = self.q[l][k] + (1/self.link_set.links[l].svcRate)
