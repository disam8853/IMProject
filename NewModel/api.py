...
def create_switch_adjacency_matrix():
    ...
    r_link = requests.get('https://140.112.106.237:16904/api/openflow/link', auth=('sdbox', 'sdbox'), verify=False)
...
r_switch = requests.get('https://140.112.106.237:16904/api/openflow/switch', auth=('sdbox', 'sdbox'), verify=False)
r_link = requests.get('https://140.112.106.237:16904/api/openflow/link', auth=('sdbox', 'sdbox'), verify=False)
...
if r_switch.status_code == requests.codes.ok:
    ...
    link_data = json.loads(r_link.text)
    decision = pd.read_excel('E:\\sdn\\decision.xlsx', sheet_name='FCFS')
    f = open('E:\\sdn\\flowentry.txt', 'w')
    f.close()
    for i in range(len(decision)):
        ...
        for n in range(len(path)-1):
            ...
            params = [{"sw": switch_id, "priority": priority, "match":{"ipv4_src":origin_ip, "ipv4_dst":destination_ip}, "actions":[{"type":"OUTPUT", "port":output_port}], "groups":1, "table_id":1}]
            r_add_flowentry = requests.post('https://140.112.106.237:16904/api/openflow/flowentry/', auth=('sdbox', 'sdbox'), verify=False, headers=headers, json=params)
            print(params)
            response = json.loads(r_add_flowentry.text)
            # 將新增的 flow entry 記錄在文字檔中
            flow_id = response[0]['id']
            f = open('E:\\sdn\\flowentry.txt', 'a')
            f.write(flow_id + "\n")
            f.close()
...
# 查看flow entry是否有正確新增
r_flowentry = requests.get('https://140.112.106.237:16904/api/openflow/flowentry', auth=('sdbox', 'sdbox'), verify=False)
...
f = open('E:\\sdn\\flowentry.txt', 'r')
for flow_id in f.readlines():
    flow_id = flow_id.strip()
    r_del_flowentry = requests.delete('https://140.112.106.237:16904/api/openflow/flowentry/'+flow_id+'/', auth=('sdbox', 'sdbox'), verify=False)
f.close()