...
def create_nodes(r_num: int, o_num: int, p_num: int, q_num: int, s_num: int):
    ...
    r_in = NodeSet(); r_out = NodeSet(); o_in = NodeSet(); o_out = NodeSet(); p_in = NodeSet(); p_out = NodeSet()
    q_in = NodeSet(); q_out = NodeSet(); s_in = NodeSet(); s_out = NodeSet()
    idx = 0
    # id從r開始往上加
...
def create_location_list(r_in: NodeSet, r_out: NodeSet, o_in: NodeSet, o_out: NodeSet, p_in: NodeSet, 
    ...
    r_loc = LinkSet(); o_loc = LinkSet(); p_loc = LinkSet(); q_loc = LinkSet(); s_loc = LinkSet()
    arti_nodes = LinkSet()

    for i in range(r_in.length()):
        node = Link(r_in.get_node(i).id, 'arti', r_in.get_node(i), r_out.get_node(i), 1)
        r_loc.add_link(node)
        arti_nodes.add_link(node)
    
    for i in range(o_in.length()):
        node = Link(o_in.get_node(i).id, 'arti', o_in.get_node(i), o_out.get_node(i), 1)
        o_loc.add_link(node)
        arti_nodes.add_link(node)

    for i in range(p_in.length()):
        node = Link(p_in.get_node(i).id, 'arti', p_in.get_node(i), p_out.get_node(i), 1)
        p_loc.add_link(node)
        arti_nodes.add_link(node)
    
    for i in range(q_in.length()):
        node = Link(q_in.get_node(i).id, 'arti', q_in.get_node(i), q_out.get_node(i), 1)
        q_loc.add_link(node)
        arti_nodes.add_link(node)
    
    for i in range(s_in.length()):
        node = Link(s_in.get_node(i).id, 'arti', s_in.get_node(i), s_out.get_node(i), 1)
        s_loc.add_link(node)
        arti_nodes.add_link(node)
...
def generate_comm_link(r_loc: LinkSet, o_loc: LinkSet, p_loc: LinkSet, q_loc: LinkSet, s_loc: LinkSet):
    idx = r_loc.length() + o_loc.length() + p_loc.length() + q_loc.length() + s_loc.length()
    comm_linkset = LinkSet()
    adjacency = Adjacency()
    #weights = [] # mu12 init value
    
    def add_comm(node1, node2, capacity_idx):
        comm_linkset.add_link(Link(comm_linkset.get_id(), 'comm', node1, node2, capacity_idx))
