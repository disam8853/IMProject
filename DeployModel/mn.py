...
class Catknight(MininetTopo):
    ...
    def feed(self, nodes=[], links=[]):
        ...
        for node in nodes:
            self.switches[node] = {}
            self.switches[node]['obj'] = self.net.addSwitch(node, dpid=int2dpid(
                dpid_i), protocols='OpenFlow13', datapath='user', ovs='ovsk')
            # unused port number
            self.switches[node]['unused'] = 3
            self.switches[node]['dpid'] = dpid_i
            self.dpids.append(dpid_i)
            self.switches[node]['priority'] = 1000
            self.switches[node]['unused_gid'] = 1
            self.switches[node]['multioutput'] = {}
            printlog(f'10.0.{ip_i}.0/24')
            self.switches[node]['host1'] = self.net.addHost(
                f"{node}_h1", ip=f'10.0.{ip_i}.1', mac=self._new_mac())
            self.switches[node]['host2'] = self.net.addHost(
                f"{node}_h2", ip=f'10.0.{ip_i}.2', mac=self._new_mac())
            ip_i = ip_i + 1
            self.net.addLink(
                self.switches[node]['obj'], self.switches[node]['host1'], port1=1, port2=1)
            self.net.addLink(
                self.switches[node]['obj'], self.switches[node]['host2'], port1=2, port2=1)
            dpid_i = dpid_i + 1
        ...
        for (a, b) in links:
            ...
            if self.linkmap.get(b) == None:
                self.linkmap[b] = {}
            self.linkmap[a][b] = (self.switches[a]['unused'],
                                  self.switches[b]['unused'])
            self.linkmap[b][a] = (self.switches[b]['unused'],
                                  self.switches[a]['unused'])
            self.switches[a]['unused'] = self.switches[a]['unused'] + 1
            self.switches[b]['unused'] = self.switches[b]['unused'] + 1

        ctrl_1 = self.net.addController(
    ...
    def addPath(self, path=[]):
        ...
        for (a, b) in zip(path, path1):
            if a not in self.linkmap or b not in self.linkmap[a]:
                raise Exception(
                    f"One switches in Link {a} to {b} doesn't exist")
        ...
        self.switches[path[-1]
                      ]['priority'] = self.switches[path[-1]]['priority'] + 1
        # head endpoint arp
        fe.append({
        ...
        self.switches[path[0]
                      ]['priority'] = self.switches[path[0]]['priority'] + 1
        # tail endpoint arp
        fe.append({
            'sw': self.switches[path[-1]]['id'],
            'priority': self.switches[path[-1]]['priority'],
        ...
        if ipv4_dst not in self.switches[path[0]]['multioutput']:
            ...
            self.switches[path[0]
                          ]['multioutput'][ipv4_dst]['gid'] = self.switches[path[0]]['unused_gid']
            self.switches[path[0]
                          ]['unused_gid'] = self.switches[path[0]]['unused_gid'] + 1
            ...
            self.switches[path[0]
                          ]['multioutput'][ipv4_dst]['priority'] = self.switches[path[0]]['priority']
            self.switches[path[0]
                          ]['priority'] = self.switches[path[0]]['priority'] + 1

        if self.switches[path[0]]['multioutput'][ipv4_dst]['feid'] != '':
            resp = self.s.delete(
                f"https://192.168.11.232/api/openflow/flowentry/{self.switches[path[0]]['multioutput'][ipv4_dst]['feid']}/")
            self.switches[path[0]]['multioutput'][ipv4_dst]['feid'] = ''
            printlog('Delete head action group fe')
            printlog(resp)
        if self.switches[path[0]]['multioutput'][ipv4_dst]['geid'] != '':
            resp = self.s.delete(
                f"https://192.168.11.232/api/openflow/groupentry/{self.switches[path[0]]['multioutput'][ipv4_dst]['geid']}/")
            printlog(
                f"Delete ge {self.switches[path[0]]['multioutput'][ipv4_dst]['geid']}")
            printlog(f"{resp.status_code}: {resp.text}")
        self.switches[path[0]]['multioutput'][ipv4_dst]['ports'].append(
            self.linkmap[path[0]][path[1]][0])

        # add group entry
        fsw = self.switches[path[0]]
        ge = [self.multioutputge(fsw['id'], fsw['multioutput']
                                 [ipv4_dst]['gid'], fsw['multioutput'][ipv4_dst]['ports'])]
        printlog('Groupentry')
        printlog(ge)
        ...
        while 1:
            try:
                resp = self.s.post(
                    'https://192.168.11.232/api/openflow/groupentry/', json=ge)
        ...
        printlog(hagfe)
        resp = self.s.post(
            'https://192.168.11.232/api/openflow/flowentry/', json=hagfe).json()
        ...
        for i in range(1, len(path)-1):
            ...
            self.switches[path[i]
                          ]['priority'] = self.switches[path[i]]['priority'] + 1
            fe.append({
                'sw': self.switches[path[i]]['id'],
                'priority': self.switches[path[i]]['priority'],
                'table_id': 1,
        ...
        printlog(fe)
        resp = self.s.post(
            'https://192.168.11.232/api/openflow/flowentry/', json=fe).json()
        printlog("add  Path")
        printlog(resp)
        printlog("\n")
    ...
    def pingFull(self, n1, n2):
        h1 = self.switches[n1]['host1']
        h2 = self.switches[n2]['host1']
        print('ping')
        print(h1, h2)
        self.net.pingFull([h1, h2])
    ...
    def addLanRoute(self):
        ...
        printlog(fe)
        resp = self.s.post(
            'https://192.168.11.232/api/openflow/flowentry/', json=fe)
        printlog(resp.json())
    ...
    def test(self):
        ...
        s101 = self.net.addSwitch('s101', dpid=int2dpid(
            101), protocols='OpenFlow13', datapath='user', ovs='ovsk')
        s102 = self.net.addSwitch('s102', dpid=int2dpid(
            102), protocols='OpenFlow13', datapath='user', ovs='ovsk')
        s103 = self.net.addSwitch('s103', dpid=int2dpid(
            103), protocols='OpenFlow13', datapath='user', ovs='ovsk')
        h1 = self.net.addHost('s101_h1', ip='10.0.1.1/24',
                              mac=self._new_mac(), defaultRoute='via 10.0.1.254')
        h2 = self.net.addHost('s101_h2', ip='10.0.1.2/24',
                              mac=self._new_mac(), defaultRoute='via 10.0.1.254')

        printlog('create switches links')
        self.net.addLink(s101, h1, port1=1, port2=1)
        self.net.addLink(s101, h2, port1=2, port2=1)

        printlog('create controllers')
        ctrl_1 = self.net.addController(
            'c1', controller=RemoteController, ip='192.168.11.232', port=6633)
        self.net.build()
